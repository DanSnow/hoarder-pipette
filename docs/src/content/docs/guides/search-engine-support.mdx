...
<ul>
  ...
  <IconDescription state="partial">
    The search engine is partially enabled. Partial means that Hoarder's Pipette is enabled on some, but not all of the
    url's that are associated with that search engine.
  </IconDescription>
